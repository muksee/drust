...
//! -verify_return_error
//!
//! 不加客户端证书报错：alert certificate required
//! 客户端证书撤销报错：alert certificate revoked
//! ```
...
fn main() {
    ...
    let listener =
        std::net::TcpListener::bind(format!("[::]:{}", args.port)).unwrap();
    for stream in listener.incoming() {
        ...
        let config =
            test_pki.server_config(&args.crl_path, accepted.client_hello());
